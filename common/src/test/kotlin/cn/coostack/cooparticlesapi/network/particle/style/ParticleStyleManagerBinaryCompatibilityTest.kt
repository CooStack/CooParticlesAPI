...
class ParticleStyleManagerBinaryCompatibilityTest {
    ...
    @Test
    fun `legacy shader pipe managers remain available for existing consumers`() {
        val classFile = projectFile(
            "common/build/classes/kotlin/main/cn/coostack/cooparticlesapi/renderer/client/ShaderPipeManagers.class"
        )

        val defaultAccess = readMethodAccess(
            classFile,
            "getDefault",
            "()Lcn/coostack/cooparticlesapi/renderer/shader/pipe/manager/ShaderPipeManager;"
        )
        val simpleBloomAccess = readMethodAccess(
            classFile,
            "getSimpleBloom",
            "()Lcn/coostack/cooparticlesapi/renderer/shader/pipe/manager/ShaderPipeManager;"
        )

        assertTrue(defaultAccess and ACC_PUBLIC != 0)
        assertFalse(defaultAccess and ACC_STATIC != 0)
        assertTrue(simpleBloomAccess and ACC_PUBLIC != 0)
        assertFalse(simpleBloomAccess and ACC_STATIC != 0)
    }

    @Test
    fun `legacy shader pipe manager methods remain available for existing consumers`() {
        val classFile = projectFile(
            "common/build/classes/kotlin/main/cn/coostack/cooparticlesapi/renderer/shader/pipe/manager/ShaderPipeManager.class"
        )
        ...
        assertTrue(
            readMethodAccess(
                classFile,
                "writeFrame",
                "(Ljava/lang/Runnable;)Lcn/coostack/cooparticlesapi/renderer/shader/pipe/manager/ShaderPipeManager;"
            ) and ACC_PUBLIC != 0
        )
        assertTrue(readMethodAccess(classFile, "render", "()V") and ACC_PUBLIC != 0)
    }

    @Test
    fun `legacy render entity manager methods remain available for existing consumers`() {
        val classFile = projectFile(
            "common/build/classes/kotlin/main/cn/coostack/cooparticlesapi/renderer/client/ClientRenderEntityManager.class"
        )
        ...
        assertTrue(
            readMethodAccess(
                classFile,
                "register",
                "(Lnet/minecraft/resources/ResourceLocation;Lnet/minecraft/network/codec/StreamCodec;)V"
            ) and ACC_PUBLIC != 0
        )
        assertTrue(
            readMethodAccess(
