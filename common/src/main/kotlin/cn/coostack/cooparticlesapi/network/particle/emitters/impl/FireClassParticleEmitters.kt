...
class FireClassParticleEmitters(var player: UUID, pos: Vec3, world: Level?) : ClassParticleEmitters(pos, world) {
    ...
    companion object {
        ...
        val CODEC = StreamCodec.of<FriendlyByteBuf, ParticleEmitters>(
            { buf, data ->
                ...
                val instance = FireClassParticleEmitters(player, Vec3.ZERO, null)
...
//            updatePhysics(controler.particle.pos, data)
            ...
            if (data.velocity.y >= maxY) {
                data.velocity = Vec3(data.velocity.x, maxY, data.velocity.z)
            }
            this.size = 0.3f
