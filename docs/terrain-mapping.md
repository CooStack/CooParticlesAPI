...
`screenOnly()` 适合整体压暗、染色、径向扩散和屏幕空间遮罩；需要真实 block atlas、面法线、cutout alpha 或只对指定 terrain quad 输出的图案，仍使用普通 world terrain Mapping。screen post 没有 block atlas 的 alpha 信息。Iris 下 `SceneDepth` 是 shader pack final 的场景深度，`SceneDepthNoHand` 是手部绘制前的深度，三个 terrain snapshot 则分别描述 opaque terrain 和 translucent terrain 的绘制边界。shader 应同时检查 terrain snapshot 与最终深度，且要求最终深度未在 hand 阶段改变，借此排除原版实体和第一人称手。screen Terrain Mapping 活跃时，非 Iris 路径会暂存 CParticle，先处理不含粒子的背景，再按原 layer、混合、排序和 deferred-depth 语义重放；Iris shader pack 路径保留全部 CParticle 在原生粒子阶段，由 Iris 的粒子 framebuffer 负责 shaderpack 附件和 Bloom，同时关闭透明 CParticle 的深度写入，避免 Mapping 把粒子深度当成 terrain 深度。Iris 路径会在 final compute 完成并执行 memory barrier 后、final 输入生成 mipmap 前执行 Mapping，确保 final fragment 的普通采样和多级采样都读取新结果。API 会读取 final program 的活跃 `sampler2D`，识别 `colortexN` 及 OptiFine 旧别名，选出 final shader 实际使用的场景颜色 attachment，再按 Iris 的 main/alt ping-pong 方向回写对应纹理；多个非零候选无法安全区分时不改写辅助附件，改走 final 后的兼容路径。`CParticleCoverageMask` 与 `CooHasCParticleCoverage` 现在保存隔离后的粒子颜色与连续 Alpha，Mapping 先按自身的空间 mask 和 Blackness 完整处理 terrain，再只叠回粒子颜色，不再把粒子后方 terrain 的原始颜色恢复。所有写入最终屏幕的 Mapping 节点必须声明并连接 coverage 输入。`postInScene()` 的顺序仍由对应 backend 管理；深度纹理不可用时，required 输入会跳过该 pass，不能重新引入共面 terrain overlay。
